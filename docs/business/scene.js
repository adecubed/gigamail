...
export function startScene({reduced,paused}) {
 ...
 const colors=[0x58e0d0,0xc8b8ff,0xffbf8b,0xc8b8ff,0xc8b8ff,0xffbf8b,0xb4d4c5,0xc0c0cc];
 ...
 const linePositions=new Float32Array(7*6),lineGeometry=new THREE.BufferGeometry();lineGeometry.setAttribute('position',new THREE.BufferAttribute(linePositions,3));const lineMaterial=new THREE.LineBasicMaterial({color:0x58e0d0,transparent:true,opacity:.3});group.add(new THREE.LineSegments(lineGeometry,lineMaterial));
 ...
 const particleGeometry=new THREE.BufferGeometry();particleGeometry.setAttribute('position',new THREE.BufferAttribute(particlePositions,3));const particleMaterial=new THREE.PointsMaterial({color:0xa69cf4,size:.045,transparent:true,opacity:.7});group.add(new THREE.Points(particleGeometry,particleMaterial));
 ...
 const stage=p<.4?0:p<.67?1:2;if(stage!==lastStage){lastStage=stage;document.querySelector('#stage-label').textContent=['EMAIL SPARSE','ORGANIZZA E COLLEGA','PRONTO PER LAVORARCI'][stage];document.querySelector('#stage-title').innerHTML=['I messaggi sono sparsi.<br>Il lavoro è connesso.','La posta trova<br>il suo contesto.','Organizza. Trova.<br>Comprendi. Agisci.'][stage];document.querySelector('#stage-description').textContent=['Ogni email contiene un pezzo della stessa storia.','Messaggi collegati per cliente, progetto e pratica.','Il tuo agente prepara il prossimo passo. Tu controlli le azioni sensibili.'][stage];}
 if(mobile()){host.style.top=(540*(1-transform)+innerHeight*.37*transform)+'px';host.style.height=(330*(1-transform)+innerHeight*.4*transform)+'px';}else{host.style.left=(44*(1-transform))+'%';host.style.top=(16+15*transform)+'%';host.style.height=(73-17*transform)+'%';}
